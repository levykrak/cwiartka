...
Kjs:
   ...
   Do
      ...
      If Tft = "R" Then
         Print #1 , "tsw b1,0" ; Chr(&Hff) ; Chr(&Hff) ; Chr(&Hff);       ' blokujemy przycisk START
         Print #1 , "b1.pco=65535" ; Chr(&Hff) ; Chr(&Hff) ; Chr(&Hff);       ' blokujemy przycisk START
         Print #1 , "tsw b1,1" ; Chr(&Hff) ; Chr(&Hff) ; Chr(&Hff);       ' blokujemy przycisk cancel
         ...
         Loop Until Tft = "C" Or Czas_meta_lewa > 0         'or zakonczony przejazd dopisac
      End If
...
Sub Write_to_sd_cwiartka()                                  ''()
   ...
   If Gbdriveerror = 0 Then
      ...
      Else
         ...
         Open "cwiartka.txt" For Append As #3               'otwórz plik pomiar.txt aby dopisaæ dane
...
Sub Write_to_sd_kjs()                                       ''()
   ...
   If Gbdriveerror = 0 Then
      ...
      Else
         Reset Sd_dioda                                     'Zapala diodê LED
         Open "KJS.txt" For Append As #3                    'otwórz plik pomiar.txt aby dopisaæ dane
         Write #3 , No_left , Czas_meta_lewa
         Flush #3                                           'zapisz bufor pliku na karcie SD
         Close #3                                           'zamknij kana³ transmisji sprzêtowego urz¹dzenia
         Blad_sd = 0                                        'ustaw zmienna na 0
         Set Sd_dioda                                       'zgaœ diodê LED
...
Isr_meta_lewa:
   ...
   If Meta_lewa = 1 Then
      ...
      Elseif Kjs_flaga = 1 Then
         ...
         Elseif Czas > 5000 And Czas_start_lewa_flaga = 1 Then
            Czas_meta_lewa = Czas
